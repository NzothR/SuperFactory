...
机器运行时不会自动修改 OC、P、duration 和 energy。

### 3.3 节点运行语义

节点每次启动时，严格按照自身固定属性运行：
...
节点不能在运行时自动降并行，也不能自动升并行。如果输入不足以满足完整 P 对应的需求，则该节点本轮不能启动。
...
7. 每个节点按照固定 OC、P、耗时、耗能运行。
8. 调度器只决定节点启动顺序，不修改节点参数。
9. 节点启动时优先消耗内部缓存，再消耗外部输入。
10. 节点完成后按物料属性路由输出。
11. 循环目标物料采用自动估计水位控制。
12. 超过循环目标水位的部分才作为净终产物输出。
13. 再次提交或停止时，所有内部缓存、主机、NC 和运行任务都必须通过输出模式清空。
```
...
该机器是一台工序图虚拟执行容器。玩家提交工序后，机器先进入输出模式清空旧状态，再进入输入模式收集每个节点独占所需的多方块主机、NC 物品，以及循环工序所需的启动物料。主机和 NC 只作为运行资格门槛，不参与每次配方执行；循环启动物料则进入内部缓存并实际参与运行。进入运行模式后，机器根据工序图节点生成虚拟运行任务，不调用官方多方块主机配方执行路径，而是由自身算法模拟节点固定 OC、P、耗时、耗能下的输入消耗和输出产出。运行过程中机器维护外部输入、内部缓存和输出缓存，优先消耗内部中间物，并对参与循环的终产物执行自动水位控制，保证循环不断流且具有净输出。停止或再次提交工序时，机器必须进入输出模式并清空全部内部状态后，才能进入待机或切换到新工序。
