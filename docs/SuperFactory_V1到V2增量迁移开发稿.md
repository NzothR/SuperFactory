...
```text
当前处于快速迭代阶段，阶段 1 不为旧运行状态、旧循环规则或旧图异常结构编写复杂迁移逻辑。
只保留 endNode 字段作为 targetProductNode 的兼容入口。
如果旧图结构与 V2 图语义冲突，可以通过日志给出 warning/error，后续由玩家重新提交或调整图。
```
